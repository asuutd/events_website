	...
	return (
		<>
					...
					<Tab.Panel
							...
							>
								<div className='flex flex-wrap gap-5 p-5 bg-white'>
									{upcoming.length > 0 ? upcoming.map((ticket) => (
										<TicketCard priority={true} key={ticket.id} ticket={ticket} onClick={() => {
							...
							>
								<div className='flex flex-wrap gap-8 bg-white'>
									{past.length > 0 ? past.map((ticket) => (
										<TicketCard priority={false} key={ticket.id} ticket={ticket} onClick={() => {
											setSelectedTicket(ticket.id);
											setIsOpen(true);
										}} />
			...
			<Modal isOpen={isOpen} closeModal={() => setIsOpen(false)}>
									<TicketDetails ticket={ticket.data?.find((t)=>t.id==selectedTicket) as unknown as TicketWithEventData || undefined} />
								</Modal>
...
function TicketCard(props: {ticket: TicketWithEventData, onClick: () => void, priority: boolean}) {
	const { ticket, onClick, priority } = props;
	return <Tilt>
		<div onClick={onClick} key={ticket.id} className="border relative rounded-md bg-white my-3 w-80 hover:scale-110 transition-all text-black shadow-xl hover:shadow-2xl hover:cursor-pointer">
